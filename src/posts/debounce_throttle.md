...
- 注意:
  1. setTimeout中建议使用箭头函数,会指向函数外的this(这个this就是调用这个事件的id为container的div)
...
> 如果希望事件每次重新点击都会立刻执行,然后等到停止触发n秒后,才可以重新触发.而不是非要等到事件停止触发后才执行
...
2. `remaining <= 0 || remaining > wait`:小于等待时间,或者时间超过了等待时间(previous只有在if...内部会更新.第一次触发后只有now会不停更新)
