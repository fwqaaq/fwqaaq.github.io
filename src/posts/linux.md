...
| **/home**       | 用户的主目录,在linux中共每个用户都有一个自己的目录,一般该目录是由用户账号命名 |
...
- `firewall` 的底层是使用 `iptables` 进行数据过滤,建立在 `iptables`
  之上,这可能会与 `Docker` 产生冲突.

当 firewalld 启动或者重启的时候,将会从 iptables 中移除 DOCKER 的规则,从而影响了
Docker 的正常工作。

当你使用的是 Systemd 的时候, firewalld 会在 Docker 之前启动,但是如果你在
`Docker` 启动之后再启 或者重启 `firewalld`,<sapn style="color:red">你就需要重启
Docker 进程了</sapn>
