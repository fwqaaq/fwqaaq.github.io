...
普通浏览器没有 x402 客户端，所以需要一个付款页。官方的 `@x402/paywall`
是一个约 2.3 MB 的内联页面，包含 React
和多条链的钱包栈，打开明显卡顿。本站改用自写的轻量付款页：不到 10
KB，原生 JavaScript 直接调用 `window.ethereum`，秒开。

当请求来自真浏览器（`Accept: text/html`）且尚未付款时，本站返回这个付款页。它做四件事：

1. **连接钱包**。支持任意注入 `window.ethereum` 的浏览器插件钱包，例如
   MetaMask、Rabby、OKX Wallet。不支持 WalletConnect 扫码。
2. **自动切换网络**。钱包里没有目标链时，会按 EIP-3085 调用
   `wallet_addEthereumChain`，先弹「添加 Base Sepolia/Base
   网络」确认，再自动切换。
3. **签名授权**。按付款要求组 EIP-712 的 `TransferWithAuthorization`
   授权并签名。这一步只是签名，不消耗 gas。
4. **提交并刷新**。把授权放进 `PAYMENT-SIGNATURE`
   头重发请求。结算成功后页面自动刷新，服务端凭通行证直接返回全文。
...
每次访问都上链结算既慢又费。所以付款成功后，服务端会做两件事。

第一，把这笔购买写进 Workers KV，key 形如
`purchase:{network}:{payer}:{slug}`。**这是购买事实的源**：付款人地址来自授权签名，无法伪造；记录永久有效，不随浏览器状态丢失。

第二，用服务端密钥做 HMAC-SHA256 签发一张**通行证**，写进带期限的
cookie。之后同一篇文章的请求带上它，服务端只需本地验签，即可跳过付款直接返回全文。通行证只是快路径缓存，丢了也不影响购买事实。

三个阶段的判断依据不同：

- **首次访问**：靠 `PAYMENT-SIGNATURE` 里的钱包签名，经 facilitator
  在链上真正转账。
- **后续访问**：靠通行证 cookie，本地验签、检查未过期且文章匹配即可。
- **cookie 丢失后**：靠 KV 里的购买记录，用钱包签名恢复（见下一节）。

付款成功的页面里还会注入一个**恢复链接**（`/access/:slug?token=`）。把它存进书签，即使换了浏览器，打开链接也能直接重发通行证。

你现在能读到这段文字，正是因为你已经付款、拿到了一张仍在有效期内的通行证。

## 换设备或丢失 cookie：恢复访问

链上确实永久记录了每笔付款，但它不能直接当身份用：一次普通的 HTTP
请求不携带任何东西能证明「发请求的人控制着某个地址」。谁都可以声称自己是付款地址。所以恢复访问需要一次**钱包签名**来证明地址所有权。

流程一共四步，全程免费、不上链：

1. 付款页点「已购买过？用钱包恢复访问」，服务端签发一个 HMAC
   挑战消息，内含文章、地址和 5 分钟时效。
2. 钱包用 `personal_sign` 对挑战签名。这只是消息签名，不产生交易。
3. 服务端从签名恢复出地址，与声称的地址比对，并校验挑战未过期、未被篡改。
4. 验证通过后查 KV：该地址买过这篇文章，就重新签发通行证 cookie，刷新即见全文。

也就是说，**钱包就是账户**。清缓存、换浏览器、换电脑都不需要重新付款，只要还控制着当初付款的钱包。
