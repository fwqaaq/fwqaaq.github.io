...
>blob对象使用不可变的,原始数据的类文件对象.他的数据俺文本或者魏晋至的格式进行读取,同时也可以转换成`ReadableStream`来读取操作
...
| 方法名            | 参数      | 描述                                                                                                         |
| ----------------- | --------- | ------------------------------------------------------------------------------------------------------------ |
| readAsArrayBuffer | file/blob | 读取指定 Blob 或文件的内容.完成后`readyState`变为Done,并且触发`loadend`.`result`以`ArrayBufer`返回文件的数据 |
